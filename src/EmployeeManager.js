// ... (Imports y funciones loadData, handleMetricChange, etc. SIN CAMBIOS) ...
// ... (Solo mostramos el return modificado para brevedad, asegurate de mantener las funciones lógicas) ...

import { useState, useEffect } from 'react'
import { supabase } from './supabaseClient'

export default function EmployeeManager({ profile }) {
  // ... (Mismos estados y useEffect que antes) ...
  ...
  // ... (Copia aquí tus funciones loadData, handleMetricChange, handleSaveMetric, handleToggleStatus) ...
  // ... (Copia handleCreate, handleUpdateEmployee, handleResetPassword) ...
  // --- PEGA AQUÍ EL CONTENIDO DE TUS FUNCIONES ---
  ...
  async function loadData() {
    ...
    try {
      ...
      if ((profile.role === 'admin' || profile.role === 'gestor') && pharmacyRes.data?.length > 0 && !newPharmacyId) {
        // Opcional: auto-seleccionar la primera si se desea
  ...
  const handleCreate = async (e) => {
    ...
    try {
        if (hasLogin) {
            ...
            alert(`Usuario creado con login.`);
        } else {
  ...
  const handleUpdateEmployee = async (e) => {
      ...
      try {
          const payload = {
              ...
              pharmacy_id: parseInt(editingEmployee.pharmacy_id), role: editingEmployee.role
          };
          ...
          if (error) {
             ...
             await supabase.from('profiles').update({
                 full_name: editingEmployee.full_name, pharmacy_id: parseInt(editingEmployee.pharmacy_id)
  ...
  return (
    <div className="employee-manager">
      {(profile.role !== 'empleado') && (
        <form onSubmit={handleCreate} className="service-form">
          ...
          <div className="form-grid">
            ...
            <div><label>Rol</label>
               <select value={newRole} onChange={e=>{setNewRole(e.target.value); if(['admin','gestor','gerente'].includes(e.target.value)) setHasLogin(true);}} disabled={!canManageAll}>
             ...
             <div><label>Farmacia</label>
               <select value={newPharmacyId} onChange={e=>setNewPharmacyId(e.target.value)} disabled={!canManageAll}>
                 {!canManageAll && pharmacies.length > 0 ? <option value={pharmacies[0].id}>{pharmacies[0].name}</option> : 
                    <>
        ...
        <tbody>
          {visibleEmployees.map((emp) => (
            <tr key={emp.id} style={emp.active===false?{background:'#ffebee'}:{}}>
              <td>{emp.full_name}</td>
              <td>{emp.role} {emp.active===false && <span style={{color:'red', fontWeight:'bold'}}>(BAJA)</span>}</td>
              ...
              <td className="actions-cell">
                {(isManager && emp.active!==false) && <button className="button-secondary" onClick={() => setEditingEmployee({...emp, new_password: '', new_email: ''})}>Editar</button>}
                {(isManager && emp.active!==false && ['admin','gestor','gerente'].includes(emp.role)) && <button className="button-secondary" onClick={()=>handleResetPassword(emp.id, emp.full_name)}>Reset</button>}
                
                {/* SOLO Botón Baja/Alta, eliminado el de Borrar */}
      ...
      {/* Modal Edición (Incluido igual que antes) */}
      {editingEmployee && (
